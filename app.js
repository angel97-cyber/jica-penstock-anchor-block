...
function migrateCoordinates(c) {
    ...
    if (!c.xyCoords || !Array.isArray(c.xyCoords) || c.xyCoords.length === 0) {
        c.xyCoords = [ {x:0, y:0}, {x:3, y:0}, {x:3, y:4}, {x:0, y:4} ];
    }
    if (!c.xzCoords || !Array.isArray(c.xzCoords) || c.xzCoords.length === 0) {
        c.xzCoords = [ {x:0, z:0}, {x:6.17, z:0}, {x:6.17, z:3}, {x:0, z:3.533} ];
    }
    if (!c.yzCoords || !Array.isArray(c.yzCoords) || c.yzCoords.length === 0) {
        c.yzCoords = [ {y:0, z:0}, {y:4, z:0}, {y:4, z:3}, {y:0, z:3} ];
    }
    if (!c.pipeXY || !Array.isArray(c.pipeXY) || c.pipeXY.length === 0) {
        c.pipeXY = [ {x:0, y:2}, {x:3, y:2}, {x:6.17, y:2} ];
    }
    if (!c.pipeCenterYZ) {
        c.pipeCenterYZ = { y: 2.0, z: 1.5 };
    }
    ...
    if (!c.pipeXZ && c.pipeZ) {
        ...
        delete c.pipeZ;
    } else if (!c.pipeXZ || !Array.isArray(c.pipeXZ) || c.pipeXZ.length === 0) {
        c.pipeXZ = [
            { x: 0.0, z: 3.533 },
            { x: 3.0, z: 3.0 },
            { x: 6.17, z: 3.0 }
    ...
    if (!c.groundCoords || !Array.isArray(c.groundCoords) || c.groundCoords.length === 0) {
        c.groundCoords = [
            { x: 0.0, z: 4.5 },
    ...
    } else if (!c.cutLineCoords || !Array.isArray(c.cutLineCoords) || c.cutLineCoords.length === 0) {
        c.cutLineCoords = [ {x:0, y:2}, {x:3, y:2}, {x:6.17, y:2} ];
    }
}
